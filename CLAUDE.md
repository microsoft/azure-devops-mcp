...
- `src/tools/search.ts` imports `orgName` from `src/index.ts` — importing tool modules therefore pulls in the CLI arg parsing; tests must supply the mocks the existing suites do.
