...
describe("DomainsManager: backward compatibility and domain enabling", () => {
  ...
  describe("constructor", () => {
    it("enables all domains when no argument is provided (default behavior)", () => {
      ...
      expect(enabledDomains.size).toBe(31);
      ...
      expect(enabledDomains.has("dashboards")).toBe(true);
      ...
      expect(enabledDomains.has("notification")).toBe(true);
      expect(enabledDomains.has("security-roles")).toBe(true);
      expect(enabledDomains.has("project-analysis")).toBe(true);
    });
  ...
  describe("string input handling", () => {
    ...
    it("Error and enables all domains when an invalid domain name string is passed", () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Error: Specified invalid domain 'invalid-domain'. Please specify exactly as available domains: advanced-security, pipelines, core, repositories, search, test-plans, test-results, wiki, work, work-items, dashboards, policy, task-agent, release, wit-process, notification, security-roles, project-analysis, member-entitlement, service-endpoint, service-hooks, graph, artifacts, audit, permissions, operations, extensions, feature-management, gallery, profile, approvals"
      );
    });
  });
  ...
  describe("getAvailableDomains method", () => {
    it("returns the full list of available domains", () => {
      ...
      expect(availableDomains).toEqual([
        "advanced-security",
        "pipelines",
        "core",
        "repositories",
      ...
      expect(availableDomains.length).toBe(32);
    });
  });
  ...
  describe("edge cases and additional coverage", () => {
    ...
    it("tests comma-separated string input with 'all' keyword", () => {
      ...
      const enabledDomains = manager.getEnabledDomains();
      expect(enabledDomains.size).toBe(31); // Should enable all because 'all' is present
    });
  });
