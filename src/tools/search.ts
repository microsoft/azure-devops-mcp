...
function configureSearchTools(server: McpServer, tokenProvider: () => Promise<string>, connectionProvider: () => Promise<WebApi>, userAgentProvider: () => string) {
  server.tool(
    ...
    async ({ searchText, project, repository, path, branch, includeFacets, skip, top }) => {
      ...
      const url = isOnPrem
        ? `${baseUrl}/${orgName}/_apis/search/codesearchresults?api-version=${apiVersion}`
  ...
  server.tool(
    ...
    async ({ searchText, project, wiki, includeFacets, skip, top }) => {
      ...
      const isOnPrem = process.env["ADO_MCP_MODE"] === "onprem";
      const url = isOnPrem
        ? `${baseUrl}/${orgName}/_apis/search/wikisearchresults?api-version=${apiVersion}`
        : `https://almsearch.dev.azure.com/${orgName}/_apis/search/wikisearchresults?api-version=${apiVersion}`;
  ...
  server.tool(
    ...
    async ({ searchText, project, areaPath, workItemType, state, assignedTo, includeFacets, skip, top }) => {
      ...
      const url = isOnPrem
        ...
        : `https://almsearch.dev.azure.com/${orgName}/_apis/search/workitemsearchresults?api-version=${apiVersion}`;
