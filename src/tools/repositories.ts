...
const REPO_TOOLS = {
  ...
  commit_files: "repo_commit_files",
};
...
function configureRepoTools(server: McpServer, tokenProvider: () => Promise<string>, connectionProvider: () => Promise<WebApi>, userAgentProvider: () => string) {
  ...
  server.tool(
    REPO_TOOLS.commit_files,
    "Commit one or more file changes directly to a repository branch. Creates the branch from the default branch if it does not exist. Optionally creates a pull request after the push.",
    {
      project: z.string().describe("The project name or ID containing the repository."),
      repositoryIdOrName: z.string().describe("The repository ID or name."),
      branch: z
        .string()
        .describe(
          "The target branch short name, e.g. 'feature/test'. If the branch does not exist it is created from the repository default branch."
        ),
      commitMessage: z.string().describe("The commit message."),
      changes: z
        .array(
          z.object({
            path: z.string().describe("File path in the repository, e.g. '/docs/test.md'."),
            content: z
              .string()
              .optional()
              .describe("File content as a UTF-8 or base64 string. Not required when changeType is 'delete'."),
            changeType: z
              .enum(["add", "edit", "delete"])
              .describe("The type of change: 'add' to create a new file, 'edit' to update an existing file, 'delete' to remove a file."),
          })
        )
        .min(1)
        .describe("Array of file changes to include in the commit. At least one change is required."),
      createPullRequest: z
        .boolean()
        .optional()
        .default(false)
        .describe(
          "If true, automatically creates a pull request from the pushed branch into the repository default branch after the push succeeds. PR title is the commitMessage and description is 'Auto-generated by MCP commit tool.'."
        ),
    },
    async ({ project, repositoryIdOrName, branch, commitMessage, changes, createPullRequest }) => {
      try {
        const connection = await connectionProvider();
        const gitApi = await connection.getGitApi();

        // Step 1: Resolve repository (by ID first, then by name)
        let repository;
        try {
          repository = await gitApi.getRepository(repositoryIdOrName, project);
        } catch {
          const repos = await gitApi.getRepositories(project);
          repository = repos?.find((r) => r.name?.toLowerCase() === repositoryIdOrName.toLowerCase());
          if (!repository) {
            return {
              content: [
                {
                  type: "text",
                  text: `Error: Repository '${repositoryIdOrName}' not found in project '${project}'.`,
                },
              ],
              isError: true,
            };
        ...
        // Step 2: Resolve the target branch and obtain the current tip commit (oldObjectId)
        let oldObjectId: string;
        let branchCreated = false;

        const branchRefs = await gitApi.getRefs(repositoryId, project, "heads/", false, false, undefined, false, undefined, branch);
        const targetBranchRef = branchRefs?.find((r) => r.name === branchRefName);

        if (!targetBranchRef?.objectId) {
          // Branch does not exist – create it from the default branch HEAD
          const defaultBranchShort = defaultBranch.replace("refs/heads/", "");
          const defaultRefs = await gitApi.getRefs(
            repositoryId,
            project,
            "heads/",
            false,
          ...
          const defaultRef = defaultRefs?.find((r) => r.name === defaultBranch);
          ...
          if (!defaultRef?.objectId) {
            return {
              content: [
                {
                  type: "text",
                  text: `Error: Branch '${branch}' does not exist and default branch '${defaultBranch}' could not be resolved.`,
                },
              ],
              isError: true,
            };
          ...
          const createRefResult = await gitApi.updateRefs(
            [
              {
                name: branchRefName,
                newObjectId: defaultRef.objectId,
          ...
          if (!createRefResult?.[0]?.success) {
            const msg = createRefResult?.[0]?.customMessage ?? "Unknown error";
            return {
              content: [{ type: "text", text: `Error: Could not create branch '${branch}': ${msg}` }],
              isError: true,
            };
          }

          oldObjectId = defaultRef.objectId;
          branchCreated = true;
        } else {
          oldObjectId = targetBranchRef.objectId;
        }

        // Step 3: Map user-facing changeType strings to VersionControlChangeType enum values
        const changeTypeMap: Record<string, VersionControlChangeType> = {
          add: VersionControlChangeType.Add,
          edit: VersionControlChangeType.Edit,
          delete: VersionControlChangeType.Delete,
        };

        // Step 4: Build and execute the push
        // Example push payload:
        // {
        //   refUpdates: [{ name: "refs/heads/feature/test", oldObjectId: "<sha>" }],
        //   commits: [{
        //     comment: "commit message",
        //     changes: [{
        //       changeType: 1,           // VersionControlChangeType.Add
        //       item: { path: "/docs/test.md" },
        //       newContent: { content: "Hello", contentType: 0 }  // ItemContentType.RawText
        //     }]
        //   }]
        // }
        const pushPayload = {
          refUpdates: [{ name: branchRefName, oldObjectId }],
          commits: [
            {
        ...
        const push = await gitApi.createPush(pushPayload as any, repositoryId, project);
        ...
        // Step 5: Optionally create a pull request
        if (createPullRequest && branchRefName !== defaultBranch) {
          const pullRequest = await gitApi.createPullRequest(
            {
              sourceRefName: branchRefName,
              targetRefName: defaultBranch,
              title: commitMessage,
              description: "Auto-generated by MCP commit tool.",
              isDraft: false,
              supportsIterations: true,
            },
            repositoryId,
          ...
          result.pullRequest = trimPullRequest(pullRequest, true);
        }
        ...
        let errorCategory = "Error committing files";
        if (
        ...
        } else if (message.includes("conflict") || message.includes("TF401179") || message.includes("already exists at")) {
          errorCategory = "Merge conflict or concurrent update";
        } else if (message.toLowerCase().includes("branch") && message.toLowerCase().includes("not found")) {
          errorCategory = "Branch not found";
        }

        return {
          content: [{ type: "text", text: `${errorCategory}: ${message}` }],
          isError: true,
        };
      }
    }
